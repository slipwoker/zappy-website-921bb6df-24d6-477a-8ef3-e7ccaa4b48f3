document.addEventListener('DOMContentLoaded', function () {
  ...
  document.body.addEventListener('submit', function (e) {
    ...
    fields.forEach(function (field) {
      ...
      if (value !== '') {
        ...
        if (type === 'tel' && !isValidPhone(value)) {
          showFieldError(field, getErrorMessage('tel', name));
          isValid = false;
          return;
        }
        ...
        if (minLength && value.length < parseInt(minLength, 10)) {
          showFieldError(field, getErrorMessage('minlength', name, minLength));
          isValid = false;
          return;
        }
        ...
        if (maxLength && value.length > parseInt(maxLength, 10)) {
          showFieldError(field, getErrorMessage('maxlength', name, maxLength));
          isValid = false;
          return;
        }
      }
    ...
    if (isValid) {
      ...
      if (firstError) {
        firstError.closest('.form-group, .field-wrapper, div') && 
  ...
  function getErrorMessage(type, fieldName, extra) {
    const messages = {
      required: 'This field is required.',
      email: 'Please enter a valid email address.',
      tel: 'Please enter a valid phone number.',
      minlength: 'Minimum ' + extra + ' characters required.',
      maxlength: 'Maximum ' + extra + ' characters allowed.'
    };
  ...
  function showFieldError(field, message) {
    ...
    field.addEventListener('input', function clearOnInput() {
      ...
      const err = parent && parent.querySelector('.field-error');
      if (err) err.remove();
  ...
  const animatableElements = document.querySelectorAll(OBSERVED_SELECTORS.join(', '));
  ...
  if (animatableElements.length > 0) {
    if ('IntersectionObserver' in window) {
      ...
      animatableElements.forEach(function (el) {
        el.classList.add('fade-in-ready', ANIMATION_CLASS);
      });
    }
  }
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "החלה שניצל של טוהר משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const payload = {
                    websiteId: '921bb6df-24d6-477a-8ef3-e7ccaa4b48f3',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || '921bb6df-24d6-477a-8ef3-e7ccaa4b48f3';
            ...
            if (mobileToggle && navMenu) {
                ...
                if (phoneHeaderBtn) {
                    phoneHeaderBtn.addEventListener('click', function() {
                        ...
                        // Falls back to 0507668889 placeholder which businessInfoUpdater can replace
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '921bb6df-24d6-477a-8ef3-e7ccaa4b48f3';
